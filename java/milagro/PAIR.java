...
/* Optimal R-ate pairing */
	public static FP12 ate(ECP2 P,ECP Q)
	{
		...
		BIG x=new BIG(ROM.ROM.curveDetails.getCurveBnx());
		BIG n=new BIG(x);
		ECP2 K=new ECP2();
		FP12 lv;
...
/* GLV method */
	public static BIG[] glv(BIG e)
	{
		...
		for (i=0;i<2;i++)
		{
			t.copy(new BIG(ROM.ROM.curveDetails.getCurveW()[i]));  // why not just t=new BIG(ROM.CURVE_W[i]);
			DBIG d=BIG.mul(t,e);
			v[i]=new BIG(d.div(q));
			u[i]=new BIG(0);
		}
		...
		for (i=0;i<2;i++)
			for (j=0;j<2;j++)
			{
				t.copy(new BIG(ROM.ROM.curveDetails.getCurveSB()[j][i]));
...
/* Galbraith & Scott Method */
	public static BIG[] gs(BIG e)
	{
		...
		for (i=0;i<4;i++)
		{
			t.copy(new BIG(ROM.ROM.curveDetails.getCurveWB()[i]));
			DBIG d=BIG.mul(t,e);
			v[i]=new BIG(d.div(q));
			u[i]=new BIG(0);
		}
		...
		for (i=0;i<4;i++)
			for (j=0;j<4;j++)
			{
				t.copy(new BIG(ROM.ROM.curveDetails.getCurveBB()[j][i]));
				t.copy(BIG.modmul(v[j],t,q));
				u[i].add(q);
				u[i].sub(t);
				u[i].mod(q);
...
/* Multiply P by e in group G1 */
	public static ECP G1mul(ECP P,BIG e)
	{
		...
		if (ROM.USE_GLV)
		{
			...
			Q.copy(P);
			BIG q=new BIG(ROM.ROM.curveDetails.getCurveOrder());
			FP cru=new FP(new BIG(ROM.ROM.curveDetails.getCurveCru()));
			BIG t=new BIG(0);
			BIG[] u=glv(e);
			Q.getx().mul(cru);
