...
/* test for O point-at-infinity */
	public boolean is_infinity() {
		if (ROM.ROM.curveDetails.getCurveType()==FieldDetails.EDWARDS)
		{
...
/* Conditional swap of P and Q dependant on d */
	...
	{
		x.cswap(Q.x,d);
		if (ROM.ROM.curveDetails.getCurveType()!=FieldDetails.MONTGOMERY) y.cswap(Q.y,d);
		z.cswap(Q.z,d);
		if (ROM.ROM.curveDetails.getCurveType()!=FieldDetails.EDWARDS)
		{
			boolean bd;
...
/* Conditional move of Q to P dependant on d */
	...
	{
		x.cmove(Q.x,d);
		if (ROM.ROM.curveDetails.getCurveType()!=FieldDetails.MONTGOMERY) y.cmove(Q.y,d);
		z.cmove(Q.z,d);
		if (ROM.ROM.curveDetails.getCurveType()!=FieldDetails.EDWARDS)
		{
			boolean bd;
...
/* Test P == Q */
	public boolean equals(ECP Q) {
		...
		if (ROM.ROM.curveDetails.getCurveType()==FieldDetails.WEIERSTRASS)
		{
...
/* this=P */
	...
	{
		x.copy(P.x);
		if (ROM.ROM.curveDetails.getCurveType()!=FieldDetails.MONTGOMERY) y.copy(P.y);
		z.copy(P.z);
		INF=P.INF;
	}
/* this=-this */
...
/* Calculate RHS of curve equation */
	public static FP RHS(FP x) {
		...
		if (ROM.ROM.curveDetails.getCurveType()==FieldDetails.WEIERSTRASS)
		{ // x^3+Ax+B
			FP b=new FP(new BIG(ROM.ROM.curveDetails.getCurveB()));
			r.mul(x);
			if (ROM.ROM.curveDetails.getCurveA()==-3)
			{
				FP cx=new FP(x);
				cx.imul(3);
				cx.neg(); cx.norm();
		...
		{ // (Ax^2-1)/(Bx^2-1)
			...
			b.mul(r);
			b.sub(one);
			if (ROM.ROM.curveDetails.getCurveA()==-1) r.neg();
			r.sub(one);
		...
		if (ROM.ROM.curveDetails.getCurveType()==FieldDetails.MONTGOMERY)
		{ // x^3+Ax^2+x
			FP x3=new FP(0);
			x3.copy(r);
			x3.mul(x);
			r.imul(ROM.ROM.curveDetails.getCurveA());
			r.add(x3);
			r.add(x);
		}
		r.reduce();
...
/* set from x - calculate y from curve equation */
	public ECP(BIG ix) {
		...
		if (rhs.jacobi()==1)
		{
			if (ROM.ROM.curveDetails.getCurveType()!=FieldDetails.MONTGOMERY) y.copy(rhs.sqrt());
			INF=false;
		}
		else INF=true;
	}
...
/* convert to byte array */
	public void toBytes(byte[] b)
	{
		...
		if (ROM.ROM.curveDetails.getCurveType()!=FieldDetails.MONTGOMERY) b[0]=0x04;
		else b[0]=0x02;
...
/* convert to hex string */
	public String toString() {
		...
		affine();
		if (ROM.ROM.curveDetails.getCurveType()==FieldDetails.MONTGOMERY) return "("+x.redc().toString()+")";
...
/* this*=2 */
	public void dbl() {
		...
		if (ROM.ROM.curveDetails.getCurveType()==FieldDetails.EDWARDS)
		{
			...
			C.sqr();
			D.sqr();
			if (ROM.ROM.curveDetails.getCurveA()==-1) C.neg();
			y.copy(C); y.add(D);
			y.norm();
			H.sqr(); H.add(H);
			z.copy(y);
		...
		if (ROM.ROM.curveDetails.getCurveType()==FieldDetails.MONTGOMERY)
		{
			...
			A.copy(C); A.imul((ROM.ROM.curveDetails.getCurveA()+2)/4);
...
/* this+=Q */
	public void add(ECP Q) {
		...
		//	FP I=new FP(0);
			...
			if (ROM.ROM.curveDetails.getCurveA()==1)
			{
				E.copy(D); D.sub(C);
			}
