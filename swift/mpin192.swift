...
final public class MPIN192
{
    ...
    static public func CLIENT_2(_ X:[UInt8],_ Y:[UInt8],_ SEC:inout [UInt8]) -> Int
    {
     ...
     //   px.rsub(r)
        ...
        P.neg()
        P.toBytes(&SEC);
      //  PAIR192.G1mul(P,px).toBytes(&SEC)
        return 0
    }
   ...
   //     let q=BIG(ROM.Modulus)
    ...
    static public func KANGAROO(_ E:[UInt8]?,_ F:[UInt8]?) -> Int
    {
        ...
        for _ in 0 ..< TRAP
        {
            let i=Int(t.geta().geta().geta().getA().lastbits(8))%TS
            t.mul(table[i])
            dn+=distance[i]
        }
        gf.copy(t); gf.conj()
        ...
        while (dm-dn<Int(MAXPIN))
        {
            ...
            let i=Int(ge.geta().geta().geta().getA().lastbits(8))%TS
            ge.mul(table[i])
            dm+=distance[i]
            if (ge.equals(t))
            {
    ...
    {
        let g1=FP24.fromBytes(G1)
        let g2=FP24.fromBytes(G2)
