...
console.log("Testing response for an End-User who has a valid PIN using AJAX");
...
// Pass one request to M-Pin Auth server
function pass1()
{
  if (DEBUG){console.log("Perform Pass One");}
  ...
  // Create MPin Token
  var PIN = 1234;
  mpin.token_hex = MPINAuth.calculateMPinToken(mpin.mpin_id_hex, PIN, mpin.client_secret_hex);
  if (DEBUG){console.log("mpin.token_hex " + mpin.token_hex);}

  var date=MPIN.today();
  var request =  MPINAuth.pass1Request(mpin.mpin_id_hex, mpin.token_hex, mpin.time_permit_hex, PIN, date, null);

  // PASS1 REQUEST
  body = JSON.stringify(request);
  if (DEBUG){console.log("PASS 1 Request" + body);}
  ...
  var data;
  var requestPass1 = http.request(optionsPass1, function(res) {
    ...
    res.on('end', function () {
      var response = JSON.parse(dataPass1);
      if (DEBUG){console.log("PASS 1 Response: " + dataPass1);}
      pass2(response.y);
    });

  });

  requestPass1.write(body);
  requestPass1.end();
  requestPass1.on('error', function(e) {
    console.error(e);
  });
}


// Pass two request to M-Pin Auth server
function pass2(y)
{
  if (DEBUG){console.log("Perform Pass Two");}
  ...
  PIN2 = 1234;
  var request = MPINAuth.pass2Request(y, requestOTP, accessNumber);
  request.mpin_id = mpin.mpin_id_hex;

  // PASS2 REQUEST
  body = JSON.stringify(request);
  if (DEBUG){console.log("PASS 2 Request" + body);}
  ...
  var data;
  var requestPass2 = http.request(optionsPass2, function(res) {
    ...
    res.on('end', function () {
      var response = JSON.parse(dataPass2);
      // PASS2 RESPONSE
      if (DEBUG){console.log("PASS 2 Response: " + dataPass2);}
      verify(response);
    });

  });

  requestPass2.write(body);
  requestPass2.end();
  requestPass2.on('error', function(e) {
    console.error(e);
  });
}
...
function verify(response)
{
  ...
  var requestToken = http.request(optionsToken, function(res) {
    ...
    res.on('end', function () {
      ...
      try
        {
          assert.equal('0', response.token.successCode, "Authentication Failed");
        }
      catch(err)
        {
          txt="Error description: " + err.message;
